...
export default function* OllamaAIWorkerSaga(ctx: Bot): Generator<any, void, any> {  
...
function* buildGameState(ctx: Bot): Generator<any, GameState, any> {  
  ...
  const playerTank = tanks.find(tank => tank.side !== aiTank.side)?.toObject()  

  // 收集敌人坦克信息
  const enemyTanks = tanks
    .filter(tank => tank.side !== aiTank.side)
    .map(tank => ({
      x: tank.x,
      y: tank.y,
  ...
  // 收集子弹信息
  const bulletsInfo = bullets.toArray().map(bullet => {
    const ownerTank = tanks.find(tank => tank.id === bullet.tankId)
    return {
      x: bullet.x,
      y: bullet.y,
...
// 执行AI决策  
function* executeDecision(ctx: Bot, decision: AIDecision) {  
...
// 降级为随机移动  
function* fallbackRandomMove(ctx: Bot) {  
  ...
  while (true) {  
    ...
    // 等待1-3秒  
    yield Timing.delay(randint(1000, 3000))  
  }  
}