...
import EndlessPowerUpScene from './EndlessPowerUpScene'
...
class GameScene extends React.PureComponent<GameSceneProps> {
  ...
  didMountOrUpdate() {
    ...
    if (game.status === 'idle' || game.status === 'gameover') {
      // 如果游戏还没开始或已经结束 则开始游戏
      const isEndlessMode = match.path === '/endless'
      if (isEndlessMode) {
        dispatch(actions.startEndlessGame())
      } else {
  ...
  render() {
    ...
    } else if (game.status === 'powerUpSelect') {
      return <EndlessPowerUpScene />
    } else {
