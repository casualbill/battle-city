...
import { fork, put, race, select, takeEvery } from 'redux-saga/effects'
...
// AI玩家的saga
export default function* aiPlayerSaga(playerName: PlayerName, config: PlayerConfig) {
  ...
  while (true) {
    ...
    const result = yield race({
      // AI玩家使用AI控制器而不是键盘控制器
      aiController: AIWorkerSaga(tankId),
      tank: aiTankSaga(playerName, tankId),
  ...
  function* aiTankSaga(playerName: PlayerName, tankId: TankId) {
    // AI玩家的坦克逻辑，类似于playerTankSaga但不需要键盘控制
    while (true) {
      const { tank }: State = yield select(
        (state: State) => ({ tank: state.tanks.get(tankId) }),
      )
      if (!tank || !tank.alive) {
        return tank
      }
